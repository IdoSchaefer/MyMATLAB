function viewWignerlog(U, xdomain, dt, Wrange)
    szU = size(U);
    Nx = szU(1);
    Nt = szU(2);
    ...
    figure
    axis([xdomain(1), xdomain(2), -maxP, maxP, Wrange(1), Wrange(2)])
    % The density matrix:
    ro = U(:, 1)*U(:, 1)';
    ...
    Wsurface = mesh(xint, pint, log10(abs(W)));
    xlabel('x')
    ylabel('p')
    zlabel('log(W)')
    for ti = 2:Nt
        ...
        maxW = max(max(abs(W)));
        W(abs(W)/maxW<eps) = maxW*eps;
        set(Wsurface, 'Zdata', log10(abs(W)))
    end
end