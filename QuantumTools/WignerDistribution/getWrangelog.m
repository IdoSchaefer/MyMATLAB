function Wrange = getWrangelog(U)
...
% time points, for the use of viewWigner.
    ...
    Nt = szU(2);
%     maxU = max(max(abs(U)));
%     U(abs(U)/maxU<eps) = U(abs(U)/maxU<eps)./abs(U(abs(U)/maxU<eps))*maxU*eps;
    ...
    W = fftshift(W, 1);
    maxW = max(max(abs(W)));
    W(abs(W)/maxW<eps) = maxW*eps;
    logabsW = log10(abs(W));
    Wrange = [min(min(logabsW)), max(max(logabsW))];
    for ti = 2:Nt
        ro = U(:, ti)*U(:, ti)';
        W(is_in_ro) = ro(ro_v_indices(is_in_ro));
...
%         W = real(ifft(W));
        ...
        W(abs(W)/maxW<eps) = maxW*eps;
        logabsW = log10(abs(W));
        Wtrange = [min(min(logabsW)), max(max(logabsW))];
