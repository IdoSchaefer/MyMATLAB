function [allT, allR, allnorm, allgradnorm] = allTRcoef1(V, xdomain, kdomain, Nk)
...
% allnorm: A vector that contains all the total norms in all k.
    ...
    allnorm = zeros(1, Nk);
    if nargout>3
        allgradnorm = zeros(2*Nx, Nk);
        for ki = 1:Nk
            [allT(ki), allR(ki), allnorm(ki), allgradnorm(:, ki)] = TRcoef1(k(ki), V, xdomain, Nx);
        end
    else
        for ki = 1:Nk
            [allT(ki), allR(ki), allnorm(ki)] = TRcoef1(k(ki), V, xdomain, Nx);
        end
    end 
end