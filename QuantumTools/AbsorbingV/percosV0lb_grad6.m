function [performance, gradient] = percosV0lb_grad6(opt_params, xdomain, kdomain, Nx, Nk, szpenal)
%%%%%%%%%  Applies only for Nk<Nx. It should be fixed. %%%%%%%%%%%%
...
% szpenal: The penalty parameter on the size of the potential.
    ...
    Vximag = [0; dstI([Vk((Nk_free + 1):2*Nk_free); zeros(Nx - Nk_free - 1, 1)]); 0] -...
        sqrt(2/Nx^3)*pi*sum(((1:Nk_free).').*Vk((Nk_free + 1):2*Nk_free))*((0:Nx).'); 
    ...
    gradient = [transgradVx(2:(Nk_free + 1)) - transgradVx(1); ...
        2*opt_params((Nk_free + 1):2*Nk_free).*(transgradVx((Nx + 2):(Nx + 1 + Nk_free)) -...
                     sqrt(2/Nx^3)*pi*sum(((1:Nx).').*grad_perf_Vx((Nx + 3):2*(Nx + 1)))*((1:Nk_free).'))];
%     if Nk_free == Nx
%         gradient(Nk_free) = gradient(Nk_free)/2;
%     end
end