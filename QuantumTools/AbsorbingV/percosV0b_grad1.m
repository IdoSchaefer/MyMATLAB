function [performance, gradient] = percosV0b_grad1(Vk, xdomain, kdomain, Nx, Nk, szpenal)
% The function computes the performance parameter for a cosine series
% absorbing potential. A zero boundary condition is imposed the boundaries.
% A restriction on the size of the potential can be
...
% The 0 and 1 coefficients are not included. The
...
% szpenal: The penalty parameter on the size of the potential.
    ...
    Vxreal = dctI0b([Vk(1:Nk_free); zeros(Nx - Nk_free - 1, 1)]);
    dctVkimag = dctI0b([Vk((Nk_free + 1):2*Nk_free); zeros(Nx - Nk_free - 1, 1)]); 
    Vximag = -dctVkimag.^2;
    ...
    dctgradVx = real([dctI([2*grad_perf_Vx(1); grad_perf_Vx(2:Nx); 2*grad_perf_Vx(Nx + 1)]);
                     dctI(-2*dctVkimag.*[2*grad_perf_Vx(Nx + 2); grad_perf_Vx((Nx + 3):(2*Nx + 1)); 2*grad_perf_Vx(2*(Nx + 1))])]);
    % The gradient wrt the dctI coefficients:
%    gradient = [dctgradVx(2:(Nk_free + 1)) - dctgradVx(1); dctgradVx((Nx + 3):(Nx + 2 + Nk_free)) - dctgradVx(Nx + 2)];
    gradient = [dctgradVx(3:(Nk_free + 2)); dctgradVx((Nx + 4):(Nx + 3 + Nk_free))];
    gradient(1:2:Nk_free) = gradient(1:2:Nk_free) - dctgradVx(1);
    gradient(2:2:Nk_free) = gradient(2:2:Nk_free) - dctgradVx(2);
    gradient((Nk_free + 1):2:(2*Nk_free)) = gradient((Nk_free + 1):2:(2*Nk_free)) - dctgradVx(Nx + 2);
    gradient((Nk_free + 2):2:(2*Nk_free)) = gradient((Nk_free + 2):2:(2*Nk_free)) - dctgradVx(Nx + 3);
    if Nk_free == Nx
