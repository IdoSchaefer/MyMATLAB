function [Vnew, Vnewx] = newguess0lb_sp3(Vold, Nnew)
...
% Nnew: The number of points in the new grid minus 1.
    ...
    Voldx = dctI0lb(Vold(1:Nold)) - 1i*dctI0lb(Vold((Nold + 1):2*Nold)).^2;
    Vnewx = spline((0:Nold).', Voldx, (0:Nold/Nnew:Nold).');
    Vnew = Vx2Vk0lb3([real(Vnewx(2:(Nnew + 1))); imag(Vnewx(2:(Nnew + 1)))]);
    if dim(1) == 1
        Vnew = Vnew.';
        Vnewx = Vnewx.';
    end
