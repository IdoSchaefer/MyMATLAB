function [performance, gradient] = percosV0lb_grad3(Vk, xdomain, kdomain, Nx, Nk, szpenal)
...
%     VkNim = -(Vk(Nk_free + 1)/2 + sum(Vk((Nk_free + 2):2*Nk_free)));
    ...
    if Nx>Nk_free        
        ...
        dctVkimag = dctI([Vk((Nk_free + 1):2*Nk_free); -(Vk(Nk_free + 1)/2 + sum(Vk((Nk_free + 2):2*Nk_free))); zeros(Nx - Nk_free, 1)]); %*Nxfactor); 
        Vximag = -dctVkimag.^2;
    else
        Vxreal = dctI0lb(Vk(1:Nk_free));
        dctVkimag = dctI0lb(Vk((Nk_free + 1):2*Nk_free)); 
        Vximag = -dctVkimag.^2;
    end    
