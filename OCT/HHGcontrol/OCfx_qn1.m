function [fieldt, fieldw, psi, evat, evaw, evmut, evmuw, relE, conv, niter, mallniterc, Jterms, maxgrad, alpha, invHess] = OCfx_qn1(psi0,...
...
%    iEnz = find(vfilterE>=tolprop*max(vfilterE)/10);
    ...
    % gradient of J w.r.t. fieldwnz
        ...
        allfield = dctIintgrid1(fieldw, T, t_ts(1:(Nt_ts-1)))/dctfactor;
        ...
        for allti = 1:allt_lasti
            chimupsi(allti) = -imag(allchi(:, allt_lasti - allti + 1)'*(mu_x.*allpsi(:, allti)));
        end
    ...
    % by the inhomogeneous Schreodinger equation.
        ...
        evafil = dctIintgrid1(evaw.*vfiltera, T, t_ts(1:(Nt_ts-1)))/dctfactor;
