function [fieldt, fieldw, psi, evat, evaw, evmiut, evmiuw, relE, conv, niter, mallniterc, J1, maxgrad] = OCfpnorm_evaE0b_qn(psi0, Vf, m,...
    xdomain, miux, a0, penalnormf, Dpenalnormf, fguess, filterE, filtera, orthpenal, alpha0, T, dt, Nt_ts, Nkr, tol, maxNiter)
%%%%%%% This is an old version. The gradient and the 1'st approximation to 
% the inverse Hessian aren't computed correctly. Use OCfx_qn instead. %%%%
...
%    miualpsi = zeros(Nx, Nt + 1);
    ...
    %evmiuw = zeros(1, Nt + 1);
    evafil = zeros(allt_lasti + 1, 1);
    J1fun = zeros(Nt + 1, 1);
    chimiupsi = zeros(allt_lasti, 1);
    tcheb = -cos(((1:Nt_ts) - 1)*pi/(Nt_ts-1));
    t_ts = 0.5*(tcheb+1)*dt;
    vfilterE = zeros(Nt + 1, 1);
%    vfiltermiu = zeros(1, Nwmiu + 1);
    vfiltera = zeros(Nt + 1, 1);
    for wi = 1:(Nt + 1)
...
%     plot(0:niter, real(conv(1:(niter + 1))));
    ...
    if ~isempty(alpha0)
        options.alpha0 = alpha0;
    end
    options.invHess0 = diag(vfilterE/2);
    [fieldw, ~, minus_grad, niter, ~, ~, dif_fieldw, minus_conv] = quasiNewton(@Jeval, fieldw, options);
    fieldw = fieldw.';
    grad = -minus_grad;
    maxgrad = max(abs(grad));
    conv = -minus_conv;
    relE = norm(dif_fieldw)/norm(fieldw);
...
%     grad = grad + 2*(dctIfrom_ig(chimiupsi, T, t_ts(1:(Nt_ts - 1)), igweights)*dctfactor - lambda0 - lambdaT*coswT);
    evorth = 1i*evcomT
    beep
    
    %%% Nested functions: %%%
    
    function [minusJ, minus_grad] = Jeval(fieldw)
        allfield = dctIintgrid(fieldw, T, t_ts(1:(Nt_ts-1)))/dctfactor;
        [allpsi, ~, mniterc] = solveOCkr(@ihfieldmiux, K, Vf, x, psi0, [0 T], Nt, Nt_ts, Nkr, tolprop, allfield.', miux);
        summniterc = summniterc + mniterc;
        allapsi = a0M*allpsi;
...
% The values of the expectation value are supposed to be real:
        ...
        normpsiT = psiT'*psiT;
        compsiT = ifft(K.*fft(a0.*psiT)) - a0.*ifft(K.*fft(psiT));
        evcomT = psiT'*compsiT;
        chiT = orthpenal*evcomT*compsiT + Dpenalnormf(normpsiT)*psiT;
        ...
        J2fun = -fieldw.^2./vfilterE*dw;
        J2fun([1, Nt + 1]) = J2fun([1, Nt + 1])/2;
        J2fun = J2fun(notsmall);
        minusJ = -(sum(J1fun) + sum(J2fun) + 0.5*orthpenal*evcomT^2 + penalnormf(normpsiT));
        for allti = 1:allt_lasti
            chimiupsi(allti) = -imag(allchi(:, allt_lasti - allti + 1)'*(miux.*allpsi(:, allti)));
        end
...
%        minus_grad = zeros(Nt + 1, 1);
%        minus_grad(fieldw ~= 0) = 2*fieldw(fieldw ~= 0)./vfilterE(fieldw ~= 0);
        minus_grad = 2*dw*fieldw./vfilterE;
        minus_grad(isnan(minus_grad)) = 0;
        minus_grad = minus_grad - 2*dw*(dct_chimiupsi - lambda0 - lambdaT*coswT);
    end

    function [lambda0, lambdaT] = fieldw_unc2lambda(fieldw_unc)
    ...
    % time domain, from the unconstrained field spectrum fieldw_unc.
        [lambda0, lambdaT] = fieldw_unc2lambda(fieldw_unc);
