function [fieldt, fieldw, psi, evpt, evpw, evmiut, evmiuw, relE, conv, niter, mallniterc, J1, maxgrad, weight] = OCfpnorm_evp(psi0, Vf, m,...
    xdomain, miux, penalnormf, Dpenalnormf, fguess, filterE, filterp, orthpenal, iweight, T, dt, Nt_ts, Nkr, tol, maxNiter)
...
%    miux = miufunx(x);
    pM = spdiags(p, 0, Nx, Nx);
    %maxNiter = 1e3;
    tolprop = 1e-3*tol;
    conv = zeros(1, maxNiter + 1);
    allppsi = zeros(Nx, allt_lasti);
%    miualpsi = zeros(Nx, Nt + 1);
    ...
    vfilterp = zeros(1, Nt + 1);
    for wi = 1:(Nt + 1)
        vfilterE(wi) = filterE((wi-1)*dw);
        vfilterp(wi) = filterp((wi-1)*dw);
%        fieldw(wi) = fguess((wi-1)*dw);
    end
...
%        toc
    ...
    allevpt = real(evp(allpsi, p));
    evpt = allevpt(1:(Nt_ts - 1):allt_lasti);
    evpw = dctIfrom_ig(allevpt, T, t_ts(1:(Nt_ts - 1)), igweights)*dctfactor;
...
% The values of the expectation value are supposed to be real:
        ...
        % only for Vf as a vector:
        compsiT = Vf.*ifft(p.*fft(psiT)) - ifft(p.*fft(Vf.*psiT));
        evcomT = psiT'*compsiT;
        normpsiT = psiT'*psiT;
        conv(niter + 1) = sum(J1fun) + sum(J2fun) + 0.5*orthpenal*evcomT^2 + penalnormf(normpsiT);
    end

    function get_chiihterm(allppsi)
        evpfil = dctIintgrid(evpw.*vfilterp, T, t_ts(1:(Nt_ts-1)))/dctfactor;
        chiihterm = -allppsi(:, allt_lasti:-1:1)*spdiags(evpfil(allt_lasti:-1:1).', 0, allt_lasti, allt_lasti);     
    end
