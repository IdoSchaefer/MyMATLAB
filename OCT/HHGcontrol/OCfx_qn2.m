...
%    iEnz = find(vfilterE>=tolprop*max(vfilterE)/10);
    ...
    % the chi propagation:
    chiihterm = zeros(Nx, allt_tp_lasti);
    ...
    allevmut = real(evmiu(allpsi(:, indices_excluding_tp), mu_x));
    ...
    % gradient of J w.r.t. fieldwnz
        ...
        % The values of the expectation value are supposed to be real:
            allevat(allti) = real(allpsi(:, indices_excluding_tp(allti))'*allapsi(:, indices_excluding_tp(allti)));
        end
...
%             allfield(allt_lasti:-1:1).', mu_x, chiihterm);
    ...
    % by the inhomogeneous Schreodinger equation.
        ...
        chiihterm = -allapsi(:, backwardi)*spdiags(evafil(backwardi), 0, allt_tp_lasti, allt_tp_lasti);     
    end
