...
cw_con_allt = dctIintgrid(cww_con, 1e3, t_ts(1:(Nt_ts-1)))/dctfactor;
cw_con_window_allt = cw_con_allt*0.5.*(tanh((allt-120)/30) - tanh((allt-880)/30));
allgsocw3 = zeros(1, 12);
allfields_psi3 = zeros(3, 5001, 12);
for cwi = 1:12
    cw_con_window_allti = 0.1*cwi*cw_con_window_allt;
    allpsi3i = solveOCMLSih(@ihfieldMLS, H03, [0 1], miu240_3, [1;0;0], [0 1e3], 5e3, 7, 7, 1e-5, cw_con_window_allti);
    allfields_psi3(:, 1:5000, cwi) = allpsi3i(:, 1, :);
    allfields_psi3(:, 5001, cwi) = allpsi3i(:, 7, 5000);
end
allgsocw3(:) = allfields_psi3(1, 5001, :).*conj(allfields_psi3(1, 5001, :));