function [Fi, Hts] = ihpsifMLS(psits, miu, tsi, t_ts, Tts, Nt, H0, chimiupsi, vfilterE)
    mint_ts = Tts*(tsi - 1);
    dw = pi/(Nt*Tts);
...
%    fieldw = dct(chimiupsi(1, :)).*vfilterE;
    ...
    for ti = 1:Nt_ts
%         fieldtpoint = sqrt(1/Nt)*fieldw(1) + sqrt(2/Nt)*sum(fieldw(2:Nt).*cos(wval*(mint_ts + t_ts(ti) + Tts/2)));
