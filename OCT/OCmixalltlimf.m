function [fieldt, fieldw, psi, relE, conv, niter, mallniterc, J1, maxgrad, weight] = OCmixalltlimf(psi0, target, Etarget, Vf, m, Edomain,...
...
% coeft*integ(<psi(t)U(t)|target><target|U'(t)psi(t)>), in addition to the term in
...
% target: The target state. The eigenstates are the columns, and are
% weighted such that: sum(|w|^2) = 1.
% Etarget: a vector, containing the eigenenergies of the eigenstates in target.
...
%        fieldw(wi) = fguess((wi-1)*dw);
    ...
    Ntar = size(target, 2);
    overlaps = zeros(Ntar, allt_lasti);
    overlapstate = zeros(1, allt_lasti);
    if size(Etarget, 1) == 1
        Etarget = Etarget.';
    end
    tcheb = -cos(((1:(Nt_ts - 1)) - 1)*pi/(Nt_ts-1));
    % Note that the tcheb in solveOCih is different.
    t_ts = 0.5*(tcheb+1)*dt;
    tgrid = [kron(0:dt:(T - dt), ones(1, Nt_ts - 1)) + kron(ones(1, Nt), t_ts), T];
    texp = exp(-1i*Etarget*tgrid);
    alltargets = target*texp;
...
%        tic
%        get_chiihterm();
        chiihterm = -coeft*alltargets(:, end:-1:1)*diag(overlapstate(allt_lasti:-1:1));
...
%tic
        ...
        overlaps = overlaps.*conj(texp);
        overlapstate = sum(overlaps);
%toc
% This option was found to be more than twice slowlier:
% tic
%         for tsi = 1:Nt
%             for ti = 1:(Nt_ts - 1)
%                 overlapstate((tsi - 1)*(Nt_ts - 1) + ti) = alltargets(:, (tsi - 1)*(Nt_ts - 1) + ti)'*allpsi(:, ti, tsi);
%             end
%         end
%         overlapstate(allt_lasti) = alltargets(:, allt_lasti)'*allpsi(:, Nt_ts, Nt);
% toc
        J1T = coefT*overlapstate(allt_lasti)*conj(overlapstate(allt_lasti));
        J1t = coeft*sum(integw.*overlapstate.*conj(overlapstate));
        J1 = J1T + J1t;
