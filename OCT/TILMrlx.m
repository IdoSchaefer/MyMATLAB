function [field, psi, relE, conv, niter, mallniterc, J1, maxgrad] = TILMrlx(psi0, target, E0, Edomain, miu, fguess, Epenal,...
    T, dt, Nt_ts, Ncheb, tol)
% The program uses a relxation process for an optimal control of a state to
% state transition.
% The procedure of optimization is based on a single, time-independent
% Lagrange multiplier (Chi).
...
%        tic
        ...
        for tsi = 1:Nt
            for ti = 1:(Nt_ts - 1)
                newallfield((tsi - 1)*(Nt_ts - 1) + ti) = -imag(chi'*miu*allpsi(:, ti, tsi))/Epenal;
            end
        end
        newallfield(allt_lasti) = -imag(chi'*miu*allpsi(:, Nt_ts, Nt))/Epenal;
        while flag == 0
