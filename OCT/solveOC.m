function [U, field, mniter, matvecs] = solveOC(Vtfun, K, V, Edomain, x, ui, tdomain, Nts, Nt_ts, Ncheb, tol, varargin)
...
%            end
            [Vtts, field(tsi)] = Vtfun(Ulast, x, tsi, varargin{:});
            Vhalfv = Vtts(:, tmidi);
            Vts = Vvec + Vhalfv;
%            Vtts = Vtts - Vhalfv*ones(1, Nt_ts);
            % Calculation of the inhomogeneous fi vectors:
            for ti = 1:Nt_ts
                Fi(:, ti) = -1i*(Vtts(:, ti) - Vhalfv).*Ulast(:, ti);
            end
