function [field, psi, relE, conv, niter, convprop, npropconv, mallniterc, J1, maxgrad, weight] = OCrlxcp(psi0, target, Vf, Edomain, xdomain,...
    fguess, Epenal, iweight, T, dt, Nt_ts, Ncheb, tol)
...
%        tic
        ...
        for tsi = 1:Nt
            for ti = 1:(Nt_ts - 1)
                newallfield((tsi - 1)*(Nt_ts - 1) + ti) = -imag(allchi(:, Nt_ts - ti + 1, Nt - tsi + 1)'*(x.*allpsi(:, ti, tsi)))/Epenal;
            end
        end
        newallfield(allt_lasti) = -imag(allchi(:, 1, 1)'*(x.*allpsi(:, Nt_ts, Nt)))/Epenal;
        while flag == 0
...
%        toc
    ...
    function getpsi(allfieldint)
        [allpsi, field1, mniterc] = solveOCih(@ihfield, K, Vf, Edomain, x, psi0, [0 T], Nt, Nt_ts, Ncheb, tolprop, allfieldint);
        summniterc = summniterc + mniterc;
