function [U, allfield, mniter, matvecs] = solveOCMLSihttp(ihfun, H0, Edomain, miu, ui, tdomain, Nts, Nt_ts, Ncheb, tol, allfield, varargin)
...
%    allfield = zeros(1, Nts*(Nt_ts - 1) + 1);
%    U(:, 1, 1) = ui;
...
%        Lambda(:, 1) = U(:, 1, tsi);
        ...
        while (reldif>tol && niter<Niter)
            ...
            [Fi, Hts, allfield(((tsi - 1)*(Nt_ts - 1) + 1):(tsi*(Nt_ts - 1) + 1))] = ihfun(Ulast, miu, tsi, H0, allfield, varargin{:});
%            [Fi, Hts, allfield(((tsi - 1)*(Nt_ts - 1) + 2):(tsi*(Nt_ts - 1) + 1))] = ihfun(Ulast, miu, tsi, H0, allfield, varargin{:});
