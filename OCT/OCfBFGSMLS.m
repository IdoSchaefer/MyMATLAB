function [fieldt, fieldw, psi, evmiut, evmiuw, conv, niter, mallniterc] = OCfBFGSMLS(psi0, E0, Edomain, miu, fguess, filterE, filtermiu, ...
...
%    options = optimset('gradObj', 'on', 'OutputFcn', @getconv, 'MaxIter', maxNiter);  
    % This line was changed in 31/12/17, more than six years after the
    % original one:
...
%    fieldt(Nt + 1) = allfield(Nt*(Nt_ts - 1) + 1);
    ...
    function [minusJ, minusgrad] = Jeval(fieldw)
...
%     chiihterm = zeros(Npsi, Nt_ts, Nt);
%     [evmiuw, evmiufil] = get_evmiuf;
%     for tsi = 1:Nt
%         for ti = 1:(Nt_ts - 1)
%             chiihterm(:, ti, tsi) = -evmiufil((Nt - tsi + 1)*(Nt_ts - 1) - ti + 1)*miu*allpsi(:, Nt_ts - ti + 1, Nt - tsi + 1);
%         end
%     end
%     
%     %%% Nested function: %%%
%     function [evmiuw, evmiufil] = get_evmiuf
%         psi = zeros(Npsi, Nt + 1);
%         psi(:, 1:Nt) = allpsi(:, 1, :);
%         psi(:, Nt + 1) = allpsi(:, Nt_ts, Nt);
%         evmiut = evmiuMLS(psi, miu);
%         evmiuw = dctI(evmiut);
%         evmiufil = dctIintgrid(evmiuw.*vfiltermiu, T, t_ts);
%     end
% end
