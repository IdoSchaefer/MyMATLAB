...
allconv = zeros(NT, 1001);
allniter = zeros(1, NT);
allJ1 = zeros(1, NT);
energies = zeros(1, NT);
for Ti = 1:NT
    Nti = Nt_max*allT(Ti)/allT(end);
    [allfields(Ti, 1:(Nti*(Nt_ts - 1) + 1)), fields(Ti, 1:(Nti + 1)), allpsi(:, 1:(Nti + 1), Ti), ~, convi, allniter(Ti), ~, allJ1(Ti)] = OCqn(psi0, target, H0, Edomain, miu, @(t) sin(10*pi*t), 1e-3, options, allT(Ti), dt, 7, 7, 1e-4, 1e3);
