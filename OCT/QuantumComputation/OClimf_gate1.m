function [fieldt, fieldw, psi, relE, conv, niter, mallniterc, Jterms, maxgrad, alpha, invHess] = OClimf_gate1(psi0, target, limits,...
    fJpsit, Hoperations, fixedHparams, Hdiff_matvecs, fcouplingOp, Edomain, fguess, filterE, options, T, Nt, Nt_ts, Nfm, tolprop)
...
% invHess: The inverse Hessian at the end of the optimization process
    ...
    integw_w = [dw/2; ones(Nt - 1, 1)*dw; dw/2];
...
%     chi_coup_psi = zeros(allt_lasti, Ncontrols);
    ...
    % Initializing the variables of the functional term which depends on
    % psi(t):
    chiihterm = [];
    Jpsit = 0;
    % Handling the additional time-dependent input parameters for the
    % Hamiltonian operation which are fixed during the optimization: 
    % If fixedHparams is a function handle:
    if isa(fixedHparams, 'function_handle')
        propagation_grid = [kron((0:dt:(T - dt)), ones(1, Nt_ts)) + kron(ones(1, Nt), t_ts_tp.'), T];
        fixedHparams_pgrid = fixedHparams(propagation_grid);
    else
        % If fixedHparams is a data array or an empty variable:
        fixedHparams_pgrid = fixedHparams;
    end
    % The total Hamiltonan operation time-dependent parameters, including
    % the optimized control fields:
    Hparams = [zeros(Ncontrols, allt_tp_lasti); fixedHparams_pgrid];
    % The omega grid:
    w = (0:pi/T:pi/dt).';
...
%     igweights = [2*igweights(1); igweights(2:(Nt_ts - 1))];
    integw_t = chebgridw(Nt, Nt_ts, dt);
    igweights = integw_t(Nt_ts:2*(Nt_ts - 1)).'/dt;
    % The integration weights of the omega grid points which participate in
    % the optimization:
    integwnz = integw_w(iEnz);
    ...
    fieldt = Hparams(1:Ncontrols, 1:Nt_ts:allt_tp_lasti);
    ...
    % gradient of J w.r.t. fieldwnz
        ...
        for icontrol = 1:Ncontrols
            Hparams(icontrol, :) = dctIintgrid1(fieldw(:, icontrol).', T, t_ts_tp.')/dctfactor;            
        end
        ...
        [~, mniterc, ~, max_errors_psi, history_psi] = SemiGlobalHparams(Hoperations.psi, Hoperations.diff_psi, Hdiff_matvecs,...
            Hparams, [], Edomain, psi0, [0, T], Nt, Nt_ts, Nfm, tolprop, 10, 16, test_tpoint, false);
        ...
        if ~isempty(fJpsit)
            [chiihterm, Jpsit] = fJpsit(allpsi, Nt_ts, integw_t);
            chiihterm = chiihterm(:, backwardi);
        end
        ...
        [~, mniterc, ~, max_errors_chi, history_chi] = SemiGlobalHparams(Hoperations.chi, Hoperations.diff_chi, Hdiff_matvecs,...
            Hparams(:, backwardi), chiihterm, Edomain, chiT, [T, 0], Nt, Nt_ts, Nfm, tolprop, 10, 16, tp_backward, false);
