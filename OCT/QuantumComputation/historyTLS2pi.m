...
save TLS2pi
max(field) + min(field)
[maxfield, imaxf] = max(field)
[allfield, field, psi, relE, conv, niter, mallniterc, J1, maxgrad, alpha, invHess] = OCqn([1;0], [0;1], H0, [-6 6], mu, @(t) sin(t), 1e-3, options, 2*pi, 2*pi/1e2, 7, 7, 1e-4, 1e4);
[allfield1, field1, psi1, relE1, conv1, niter1, mallniterc1, J11, maxgrad1, alpha1, invHess1] = OCqn([1;0], [0;1], H0, [-6 6], mu, @(t) maxfield*sin(t), 1e-3, options, 2*pi, 2*pi/1e2, 7, 7, 1e-4, 1e4);
[allfield1, field1, psi1, relE1, conv1, niter1, mallniterc1, J11, maxgrad1, alpha1, invHess1] = OCqn([1;0], [0;1], H0, [-6 6], mu, @(t) maxfield*sin(t), 1e-3, options, 2*pi, 2*pi/1.2e2, 7, 7, 1e-4, 1e4);
[allfield1, field1, psi1, relE1, conv1, niter1, mallniterc1, J11, maxgrad1, alpha1, invHess1] = OCqn([1;0], [0;1], H0, [-6 6], mu, @(t) maxfield*sin(t), 1e-3, options, 2*pi, 2*pi/1.5e2, 7, 7, 1e-4, 1e4);
[allfield1, field1, psi1, relE1, conv1, niter1, mallniterc1, J11, maxgrad1, alpha1, invHess1] = OCqn([1;0], [0;1], H0, [-6 6], mu, @(t) maxfield*sin(t), 1e-3, options1, 2*pi, 2*pi/1e2, 7, 7, 1e-4, 1e4);
options1 = optionsOCqn(1e-4,1e4);
options1.f_max_alpha = @(x0, direction) alpha_max_x(x0, direction, 4)
[allfield1, field1, psi1, relE1, conv1, niter1, mallniterc1, J11, maxgrad1, alpha1, invHess1] = OCqn([1;0], [0;1], H0, [-6 6], mu, @(t) maxfield*sin(t), 1e-3, options1, 2*pi, 2*pi/1e2, 7, 7, 1e-4, 1e4);
figure
J11
1-J11
plot(0:2*pi/100:2*pi, field1)
1-J1
[allfield2, field2, psi2, relE2, conv2, niter2, mallniterc2, J12, maxgrad2, alpha2, invHess2] = OCqn([1;0], [0;1], H0, [-6 6], mu, @(t) maxfield*cos(t), 1e-3, options1, 2*pi, 2*pi/1e2, 7, 7, 1e-4, 1e4);
figure
plot(0:2*pi/100:2*pi, field2)
options2 = optionsOCqn(1e-6,1e4);
options2.f_max_alpha = @(x0, direction) alpha_max_x(x0, direction, 4)
[allfield3, field3, psi3, relE3, conv3, niter3, mallniterc3, J13, maxgrad3, alpha3, invHess3] = OCqn([1;0], [0;1], H0, [-6 6], mu, @(t) 0.6*sin(t), 1e-3, options2, 2*pi, 2*pi/1e2, 7, 7, 1e-4, 1e4);
1-J13
figure
plot(0:2*pi/100:2*pi, field3)
min(field3)
format short e
min(field3)
min(field3)+maxfield
...
Dphase
Pphase'
Pphase*expm(-1i*a*Dphase)*Pphase'*[1;0]
syms a
Pphase*expm(-1i*a*Dphase)*Pphase'*[1;0]
simplify(ans)
figure
plot(0:2*pi/100:2*pi, conj(psi).*psi)
hold on
plot(0:2*pi/100:2*pi, cos((0:2*pi/100:2*pi)+0.16*pi))
plot(0:2*pi/100:2*pi, maxfield*cos((0:2*pi/100:2*pi)+0.16*pi))
delta_cos_phi = field-maxfield*cos((0:2*pi/100:2*pi)+0.16*pi);
figure
plot(0:2*pi/100:2*pi, delta_cos_phi)
t=0:2*pi/100:2*pi;
hold on
plot(t, cos(t/2))
plot(t, cos(t/4)^2)
...
syms x
int(sec(x)^2)
[allJmax, phases] = JmaxTLSphases(0.5, 100, [], 100, 7, 7, 1e-8
eig(H0+0.5*sigma_x)
eig(H0+0.5*sigma)
whos
sigma_x = [0 1; 1 0]
...
eig(H0+0.5*sigma_x)
eig(H0-0.5*sigma_x)
[allJmax, phases] = JmaxTLSphases(0.5, 100, [-0.2071, 1.2071], 100, 7, 7, 1e-8);
figure
plot(phases, allJmax)
figure
plot(phases, sin(phases))
pi/2
(2.07345-pi/2)/pi
doc plotyy
[allJmax, phases] = JmaxTLSphases(0.5, 100, [-0.2071, 1.2071], 1e3, 7, 7, 1e-8);
figure
plot(phases, sin(phases))
plot(phases, allJmax)
[allJmax, phases] = JmaxTLSphases(0.5, 1e3, [-0.2071, 1.2071], 100, 7, 7, 1e-8);
plot(phases, allJmax)
(2.08602-pi/2)/pi
figure
yyaxis left
...
xlabel('$\tau$', 'interpreter', 'latex')
yyaxis left
ylabel('$\left\Vert\left<1\vert\psi(2\pi)\right>\right\Vert$', 'interpreter', 'latex')
ylabel('$\left\Vert\left<\varphi_1\vert\psi(2\pi)\right>\right\Vert$', 'interpreter', 'latex')
ylabel('$\left\Vert\left<\varphi_1\vert\psi(2\pi)\right>\right\Vert^2$', 'interpreter', 'latex')
yyaxis right
ylabel('$\sin(\phi)$', 'interpreter', 'latex')
xlabel('$\phi$', 'interpreter', 'latex')
yyaxis left
ylabel('$\left\Vert\left<\varphi_1\vert\psi_\phi(2\pi)\right>\right\Vert^2$', 'interpreter', 'latex')
size(allfield)
chebp7 = (cos(-pi:pi/6:pi) + 1)
chebp7 = (cos(-pi:pi/3:pi) + 1)
chebp7 = (-cos(0:pi/6:pi) + 1)/2
chebtgrid = [(repmat(chebp7(1:6), ones(1, 100))+ kron(0:99, ones(1,6)))*2*pi/100, 2*pi];
chebtgrid = [(repmat(ones(1, 100), chebp7(1:6))+ kron(0:99, ones(1,6)))*2*pi/100, 2*pi];
doc repmat
chebtgrid = [(repmat(chebp7(1:6), 1,100)+ kron(0:99, ones(1,6)))*2*pi/100, 2*pi];
chebtgrid
figure
plot(chebtgrid, allfield)
(2*pi-5.77632)/pi
maxfield
maxfield = max(abs(allfield))
allJmax_maxf = JmaxTLSphases(maxfield, 1e3, [-0.2071, 1.2071], 100, 7, 7, 1e-8);
figure
plot(phases, allJmax_maxf)
(2.08602-pi/2)/pi
figure
plot(phases, allJmax_maxf)
figure
...
figure
plot(phases, allJmax_45)
allJmax_55 = JmaxTLSphases(0.55, 1e3, [-0.2071, 1.2071], 100, 7, 7, 1e-8);
figure
plot(phases, allJmax_55)
allJmax_60 = JmaxTLSphases(0.6, 1e3, [-0.2071, 1.2071], 100, 7, 7, 1e-8);
figure
plot(phases, allJmax_60)
max(allJmax)-min(allJmax)
max(allJmax_60)-min(allJmax_60)
max(allJmax_45)-min(allJmax_45)
max(allJmax_55)-min(allJmax_55)
allJmax_40 = JmaxTLSphases(0.4, 1e3, [-0.2071, 1.2071], 100, 7, 7, 1e-8);
figure
plot(phases, allJmax_40)
max(allJmax_40)-min(allJmax_40)
max(allJmax_40)
max(allJmax_45)
[maxJmax, imaxJmax] = max(allJmax)
allJmax(833)
833/1e3*2*pi
2*pi - 833/1e3*2*pi
allJmax(168)
...
maxJmax-allJmax(333)
maxJmax-allJmax(668)
maxJmax(168)-allJmax(668)
allJmax(168)-allJmax(668)
332*2*pi/1e3
[allJmax_fine, phases_fine] = JmaxTLSphases(0.5, [331, 333]*2*pi/1e3, 1e3, [-0.2071, 1.2071], 100, 7, 7, 1e-8);
[allJmax_fine, phases_fine] = JmaxTLSphasesD(0.5, [331, 333]*2*pi/1e3, 1e3, [-0.2071, 1.2071], 100, 7, 7, 1e-8);
[allJmax_fine, phases_fine] = JmaxTLSphasesD(0.5, [331, 333]*2*pi/1e3, 1001, [-0.2071, 1.2071], 100, 7, 7, 1e-8);
figure
plot(phases_fine, allJmax_fine)
[maxJmax_fine, imaxJmax_fine] = max(allJmax_fine)
phases_fine(imaxJmax_fine)
phases(333)
allJmax_fine(623)-allJmax(333)
phases_fine(imaxJmax_fine)/pi - pi/2
phases_fine(imaxJmax_fine)/pi - 0.5
1-allJmax(333)
imaximaJmax = [168 333 668 833]
allJmax(imaximaJmax)
save TLS2pi
load TLS2pi
figure
