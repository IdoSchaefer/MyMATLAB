function [field, psi, relE, conv, niter, mniterc] = OCcheb_op(psi0, targetO, Vf, Edomain, xdomain, fguess, Epenal, T, dt, Nt_ts, Ncheb, tol)
...
%            conv(:, niter+1) = overlap*conj(overlap) - Epenal*sum([0.5*field(1)^2, field(2:Nt).^2, 0.5*field(Nt + 1)^2])*dt;
            conv(:, niter+1) = psiT'*chiT - Epenal*sum(integw.*allfield.^2);
        end
...
%       conv(:, niter+1) = psi(:, Nt + 1).*conj(psi(:, Nt + 1));
        %conv(:, niter+1) = overlap*conj(overlap) - Epenal*sum([0.5*field(1)^2, field(2:Nt).^2, 0.5*field(Nt + 1)^2])*dt;
        conv(:, niter+1) = psiT'*targetO*psiT - Epenal*sum(integw.*allfield.^2);
        conv = conv(:, 1:(niter + 1));
    end
    niter
