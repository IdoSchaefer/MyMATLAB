function [fieldt, fieldw, psi, relE, conv, niter, mallniterc, J1, maxgrad, weight] = OCsuballtlimf(psi0, target, Vf, m, Edomain,...
    xdomain, coeft, coefT, fguess, filterE, iweight, T, dt, Nt_ts, Ncheb, tol)
% The program is intended to find an optimal field for a state to a subspace transition, as fast as possible.
% For this purpose, there is the following term in the functional:
% coeft*integ(<psi(t)sum(|target(i)><target(i)|)psi(t)>), in addition to the term in
...
% target: The target subspace. The vectors spanning the subspace are the columns.
...
%        toc
    ...
    function getpsi(fieldwint)
        ...
        J1T = coefT*sum(overlaps(allt_lasti)*conj(overlaps(allt_lasti)));
        J1t = coeft*sum(integw.*sum(overlaps.*conj(overlaps)));
        J1 = J1T + J1t;
