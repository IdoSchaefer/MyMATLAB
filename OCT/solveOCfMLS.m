function [U, chimiupsi, mniter, matvecs] = solveOCfMLS(ihfun, H0, Edomain, miu, ui, tdomain, Nts, Nt_ts, Ncheb, tol, chimiupsi, allconj, vfilterE, varargin)
...
% potential energy and the wave function in the sampling points.
    ...
    minE = Edomain(1);
    maxE = Edomain(2);
%    U = zeros(dim, Nt_ts, Nts + 1);
    U = zeros(dim, Nt_ts, Nts);
%     if isempty(varargin)
%         chimiupsi(1) = -imag(allconj(:, 1, 1)'*miu*ui);
%     else
%         chimiupsi(Nts + 1) = -imag(ui'*miu*allconj(:, 1, 1));
%     end
%    field = zeros(1, Nts);
%    U(:, 1, 1) = ui;
...
%        Lambda(:, 1) = U(:, 1, tsi);
        ...
        while (reldif>tol && niter<Niter)
            ...
            [Fi, Hts] = ihfun(Ulast, miu, tsi, t_ts, Tts, Nts, H0, chimiupsi, vfilterE, varargin{:});
...
%            Ctaylor = Newton2Taylor(t_ts, Cnewton);            
            ...
            if isempty(varargin)
%                 chimiupsi(:, tsi) = update_cmp(allconj(:, :, tsi), Unew, miu, Nt_ts);
                chimiupsi(tsi + 1) = -imag(allconj(:, Nt_ts, tsi)'*miu*Unew(:, Nt_ts));
            else
%                 chimiupsi(:, Nts - tsi + 1) = update_cmp(Unew(:, Nt_ts:-1:1), allconj(:, Nt_ts:-1:1, Nts - tsi + 1), miu, Nt_ts);
                chimiupsi(Nts + 1 - tsi) = -imag(Unew(:, Nt_ts)'*miu*allconj(:, Nt_ts, tsi));
            end
...
%        matvecs = matvecs + niter*(Nt_ts + Ncheb-1);
        ...
        if ~isfinite(U(1, Nt_ts, tsi))
            display('Error.');
            return
        end
    end
%    [stam, field(Nts + 1)] = Vtfun(Unew, x, varargin{:});
...
% function cmpts = update_cmp(chits, psits, miu, Nt_ts)
%     cmpts = zeros(Nt_ts, 1);
%     for ti = 1:Nt_ts
%         cmpts = -imag(chits(:, ti)'*miu*psits(:, ti));
%     end
% end