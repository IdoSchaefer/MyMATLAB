function [field, psi, relE] = OCTLS(psi0, target, E, fguess, totalfE, T, dt, tol)
...
%        chi(:, Nt+1) = conj(target).*psi(:, Nt + 1);
        ...
        for ti = 1:(Nt+1)
            ezer(ti) = 1i*(conj(chi(1, ti))*psi(2, ti) - conj(psi(1, ti))*chi(2, ti));
        end
...
% %            Vt(2, 1, ti) = -field(ti);
%         end
        field = ezer*sqrt(totalfE/integral);
