function [field, psi, relE, conv, niter, mniterc] = OCchebMLSop(psi0, targetO, E0, Edomain, miu, fguess, Epenal, T, dt, Nt_ts, Ncheb, tol)
...
%    lastfield = field;
    ...
    while relE>tol && niter<maxNiter
        ...
        chiT = targetO*psiT;
        if nargin>3
            conv(:, niter+1) = psiT'*chiT - Epenal*sum(field.^2)*dt;
        end
...
%       conv(:, niter+1) = psi(:, Nt + 1).*conj(psi(:, Nt + 1));
        conv(:, niter+1) = psiT'*targetO*psiT - Epenal*sum(field.^2)*dt;
        conv = conv(:, 1:(niter + 1));
    end
    niter
