function [fieldt, fieldw, psi, evmiut, evmiuw, relE, conv, niter, mallniterc, J1, maxgrad, weight] = OCfrlxr(psi0, Vf, m, Edomain, xdomain, ...
    fguess, filterE, filtermiu, ppenal, iweight, T, dt, Nt_ts, Ncheb, tol)
...
%        tic
        ...
        [allchi, field1, mniterc] = solveOCih(@ihfieldchif, K, Vf, Edomain, x, chiT, [T 0], Nt, Nt_ts, Ncheb, tolprop,...
...
%        toc
    ...
    function getpsi(fieldwint)
        ...
        psiT = psi(:, Nt + 1);
        evpT = psiT'*ifft(p.*fft(psiT));
        conv(niter + 1) = sum(J1fun) + sum(J2fun) - 0.5*ppenal*(evpT/m)^2;
    end

    function get_chiihterm(allpsi)
        evmiufil = dctIintgrid(evmiuw.*vfiltermiu, T, t_ts(1:(Nt_ts-1)))/dctfactor;
        for ti = 1:Nt_ts
            chiihterm(:, ti, 1) = -evmiufil(Nt*(Nt_ts - 1) - ti + 2)*x.*allpsi(:, Nt_ts - ti + 1, Nt);
        end        
        for tsi = 2:Nt
            chiihterm(:, 1, tsi) = chiihterm(:, Nt_ts, tsi - 1);
            for ti = 2:Nt_ts
                chiihterm(:, ti, tsi) = -evmiufil((Nt - tsi + 1)*(Nt_ts - 1) - ti + 2)*x.*allpsi(:, Nt_ts - ti + 1, Nt - tsi + 1);
            end
        end    
    end
