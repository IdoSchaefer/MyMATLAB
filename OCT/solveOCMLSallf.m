function [U, allfield, mniter, matvecs] = solveOCMLSallf(Vtfun, H0, Edomain, miu, ui, tdomain, Nts, Nt_ts, Ncheb, tol, varargin)
...
%        Lambda(:, 1) = U(:, 1, tsi);
        ...
        while (reldif>tol && niter<Niter)
            [Vtts, allfield(((tsi - 1)*(Nt_ts - 1) + 1):(tsi*(Nt_ts - 1) + 1))] = Vtfun(Ulast, miu, tsi, varargin{:});
            Vthalf = Vtts(:, :, tmidi);
            Hts = H0 + Vthalf;
            Htscheb = (2*Hts - (minE + maxE)*I)/(maxE - minE);
            % Calculation of the inhomogeneous fi vectors:
            for ti = 1:Nt_ts
                Fi(:, ti) = -1i*(Vtts(:, :, ti) - Vthalf)*Ulast(:, ti);
            end
