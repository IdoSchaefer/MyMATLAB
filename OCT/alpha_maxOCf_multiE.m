function max_alpha = alpha_maxOCf_multiE(fieldwnz0, direction, max_fieldt, Nt, iEnz, NEnz, Ncontrols, T)
...
% OCf_qn programs).
% T: The final time
    direction_with0 = zeros(Nt + 1, Ncontrols);
    direction_with0(iEnz, :) = reshape(direction, NEnz, Ncontrols);
    dct_direction = dctIM(direction_with0)*sqrt(Nt*pi)/T;
    fieldw0 = zeros(Nt + 1, Ncontrols);
    fieldw0(iEnz, :) = reshape(fieldwnz0, NEnz, Ncontrols);
    dct_x0 = dctIM(fieldw0)*sqrt(Nt*pi)/T;
    max_alpha = min(min((max_fieldt - sign(dct_direction).*dct_x0)./abs(dct_direction)));
end