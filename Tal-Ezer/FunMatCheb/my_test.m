...
parm(2)=2;
parm(3)=2;
% The Arnoldi algorithm doesn't work in this case:
%parm(3)=1;
parm(4)=0;
parm(5)=-iu*310;
% 
...
A=mat('matvec',n);
theta = (0:pi/(n-1):pi).';
w = sin(p)/p*L/2*sin(theta)./sqrt(1 - cos(theta).^2*sin(p)^2)*pi/n;
for j=1:k
    ex=expm(tpoints(j)*A)*v;
    er(j,1)=norm(ex-u(:,j))/(norm(ex)+1);
    er_anal(j, 1) = (sin(tpoints(j)) - u(:, j)'*(x.*w.*u(:, j))/(u(:, j)'*(w.*u(:, j))))/sin(tpoints(j));
end
er
er_anal
clear all
