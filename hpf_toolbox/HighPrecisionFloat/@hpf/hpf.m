classdef  (InferiorClasses = {?vpi}) hpf
  ...
  % exponent of 3. You can think of it in scientific notation form as
  ...
  %
  % Since I store the power of 10 associated with the number as a 
  % separate number itself, HPF numbers can become as large in magnitude
  % as that exponent will allow. The current release of HPF uses an INT64
  % integer for this purpose. Since intmax('int64') = 9223372036854775807,
  % the largest number representable in HPF form is in theory
  %
  %  .9999999999... * 10^9223372036854775807
  %
  % Older releases used a double for that exponent, so were more limited.
  %
  ...
  methods
    ...
    function A = chol(A)
      % CHOL Textbook version of Cholesky factorization for HPF input.
      %   R = chol(A), for a positive definite matrix A,
      %   is an upper triangular matrix R so that R'*R = A.
      
      % (Thanks to Cleve here, to save me the time to write this code)
      ...
      for k = 1:n
        ...
        A(k,j) = (A(k,j) - A(i,k)'*A(i,j))/A(k,k);
      end
      A = mytriu(A);
      
      % nested fun
      function A = mytriu(A)
        lowtriind = find(tril(ones(size(A)),-1));
        A(lowtriind) = 0;
      end
    ...
    function F = cot(X)
      % evaluates cot(X) (cotangent) for a hpf number X
      
      % cotangent is the reciprocal of the tangent
      F = reciprocal(tan(X));
      
    end % function F = cot(X)

    function F = cotd(X)
      % evaluates cot(X) for a hpf number X (in degrees)
      
      % simple conversion of degrees to radians
      F = X;
      for i = 1:numel(X)
        F(i) = cot(X(i).*hpf('pi',X(i).NumberOfDigits)./180);
      end
      
    end % function F = cotd(X)
    ...
    function xroot = cubrt(x)
      ...
      % reduce the problem to bring x into the interval [0.01,10)
      xroot = hpf('1',NDig);
      p = floor(x.Exponent/3);
      xroot.Exponent = int64(p + 1);
      ...
      % now lie in the half open interval [.01,10).
      ...
      while flag
        ...
        xdiff = abs(xest - xprior);
        % this test will force the iteration to happen one more time AFTER
        % it has passed the slightly sloppy convergence tolerance, to
        % make sure we squeeze out every last bit.
        if xterm && (xtol >= xdiff)
          xterm = false;
        else
          flag = false;
      ...
      % so this recovers the desired root with barely a hpf divide required.
    ...
    function d = det(X)
      % det    Determinant for HPF input
      % det(X) is the determinant of the square matrix X.
      
      % test for a square matrix
      S = size(X);
      if (length(S) ~= 2) || (diff(S) ~= 0)
        error('HPF:DET:square','X must be a square matrix for det')
      end
      
      % just use the product of the diagonals of the LU factor U
      [L,U,P] = lu(X);
      d = prod(diag(U));
    ...
    function d = diag(X,K)
      ...
      % one argument calls always work with the main diagonal
      if nargin == 1
        K = 0;
      end
      
      % is M a vector or array?
      % M a vector causes a square matrix to be created of appropriate size
      % M a matrix extracts the indicated diagonal
      if isvector(X)
        % X was a vector, so create a matrix
        N = length(X);
        ...
        % get the index for the provided elements
        ind = sub2ind([M,M],i,j);
        
        % preallocate the matrix
        % (This next line is a lazy way of creating a zero matrix with
        % the same number of digits as the elements of X.)
        d = repmat(X(1)*0,[M,M]);
        
        % Stuff X into d
        d(ind) = X;
      else
        % M was a matrix, so extract the indicated diagonal
        S = size(X);
    ...
    function Fchar = disp(F)
      ...
      elseif numel(F) > 1
        ...
        disp(['HPF array of size: ',num2str(Fsize)])
        if isloose,disp(' '),end
        ...
        for i = 1:Nf
          ...
          % now, append the number to the tag we just created
          disp([tag,disp(F(i))])
        end
        % blank line only if format loose
        if isloose,disp(' '),end
    ...
    function F = exp(X)
      ...
      % If X overflows uint64, then there will be an issue, so trap for that.
      Xiabs = abs(Xi);
      Xi64 = uint64(Xiabs);
      if Xi64 == intmax('uint64')
        F = hpf(inf,NDig);
      end
    ...
    function F = full(F)
      % full function for HPF
      
      % a no-op, since sparse is not supported for HPF
    end % F = full(F)
    ...
    function Xistr = int2str(X)
      % HPF/INT2STR Convert integer to string.
      ...
      %
      % See also hpf/num2str
      Xi = round(X);
      Xistr = num2str(Xi);
      
    end % function Xistr = int2str(X)
    ...
    function [L,U,P] = lu(A)
      %  Basic LU factorization for HPF input
      % usage: [L,U,P] = lu(A)
      %
      ...
      if (numel(s) ~= 2) || (diff(s) ~= 0)
        error('HPF:LU:Asize','A must be a square matrix')
      end
      s = s(1);
      ...
      PV=(0:s-1)';
      for j=1:s,
        % Pivoting on the columnar max value
        [~,ind] = max(abs(U(j:s,j)));
        ind = ind+(j-1);
        t = PV(j);
        PV(j) = PV(ind);
        ...
        t = L(j,1:j-1);
        L(j,1:j-1) = L(ind,1:j-1);
        L(ind,1:j-1) = t;
        t = U(j,j:end);
        U(j,j:end) = U(ind,j:end);
        U(ind,j:end) = t;
    ...
    function result = mpower(x,y)
      ...
      %
      % If x is not a scalar, then it MUST be a square array, and
      % y must be a non-negative scalar integer, no larger than
      % 2^53 - 1 as a double, or 2^64-1 as a uint64 or a larger
      % integer if hpf or vpi.
      
      % at least one of these numbers must be a hpf, so if not x then y.
    ...
    function result = num2str(N)
      % returns a string version of the supplied HPF number
      
      if nargin > 1
        warning('HPF:NUM2STR:TBD','Two argument num2str is not yet implemented.')
      end
      
      % really, just a synonym for disp here
      result = disp(N);
    end % result = num2str(N)
    ...
    function result = power(x,y)
      ...
      elseif isempty(y)
        ...
        return
      elseif (numel(x) > 1) && (numel(y) == 1)
        % scalar expansion of x
        result = repmat(hpf(0),size(x));
        for i = 1:numel(x)
          result(i) = x(i).^y;
        end
        return
      elseif (numel(y) > 1) && (numel(x) == 1)
        % scalar expansion of y
        result = repmat(hpf(0),size(y));
        for i = 1:numel(y)
          result(i) = x.^y(i);
        end
        return
      elseif (numel(x) > 1) && (numel(x) > 1)
        % both x and y are arrays
        if ~isequal(size(x),size(y))
          error('HPF:POWER:impropersize','Matrix dimensions must agree.')
        end
      ...
      % if we drop down, then both x and y are scalars
      ...
      % Some special cases to check on x. We have already
      % excluded the inf and nan cases on both parameters.
      if x == 0
      ...
      % if we drop though, then x is just some boring number.
      
      % if y is a small integer (hpf or not), then we can
      % simply convert to binary.
      if (abs(y) > (2^52)) || ((y - fix(y)) ~= 0)
        % y is too large or non-integer
        y = hpf(y,NDig);
        if x > 0
          result = exp(y.*log(x));
        else
          error('HPF:POWER:negativex', ...
            'Raising negative to a real power. Complex results not supported.')
        end
    ...
    function [N,D] = rat(X,tol)
      %RAT    Rational approximation. (for HPF input)
      ...
      if nargin < 2
        ...
        tol = sqrt(eps(max(abs(Xfinite(:)))));
      end
      if nargout > 1
        N = repmat(hpf(0,X(1).NumberOfDigits),size(X));
        D = N;
      else
        N = '';
      end
      for j = 1:numel(X)
        ...
        if ~isfinite(x), % Special case for inf, -inf, NaN
          ...
          while 1
            ...
            if ~isinf(x),
              x = x - d;
              C = [C*[d;hpf(1,d.NumberOfDigits)] hpf(C(:,1),d.NumberOfDigits)];
    ...
    function result = rdivide(num,den)
      ...
      elseif (numel(num) > 1) && (numel(den) > 1)
        ...
        if isa(num,'hpf')
          ...
          result = hpf(num,den(1).NumberOfDigits);
        end
        for i = 1:numel(den)
    ...
    function R = reciprocal(D)
      ...
      if D.Sign == 0
        ...
        R = hpf('inf',NDig);
        return
      elseif isnan(D.Numeric)
        % NaN begets NaN
        R = D;
        return
      elseif isinf(D.Numeric)
        % 1/inf = 0, 1/(-inf) = 0
        R = hpf('0',NDig);
        return
