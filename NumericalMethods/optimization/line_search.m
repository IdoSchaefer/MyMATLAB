function [xalpha, falpha, fgrad_xalpha, sol_achieved, nfevals] = line_search(f, fgrad, x0, f0, fgrad0, direction,...
    alpha1, ro, sigma, tau1, tau2, tau3, minf, maxNiter)
...
% data.
%%% Input: %%%
% f: a function handle which represents the minimized function
% fgrad: a function handle which represents the gradient of the minimized function
...
% reference above
% minf: Defined as \bar{f} in the reference above; if irrelevat, substitute -inf.
...
% nfevals: The number of function and gradient evaluations
    ...
    mu = (minf - f0)/(ro*df0_dalpha);
    alpha_last = 0;
    falpha_last = f0;
    Dfalpha_last = df0_dalpha;
    niter_bracket = 0;
    while ~accepted_interval && niter_bracket<maxNiter
        xalpha = x0 + alpha*direction;
        falpha = f(xalpha);
        fgrad_xalpha = fgrad(xalpha);
    ...
    %%% Sectioning phase: %%%
    niter_section = 0;
    while ~accepted_point && 2*(b - a)*Dfa/(abs(fa) + abs(fb))<-10*eps && niter_section<maxNiter
