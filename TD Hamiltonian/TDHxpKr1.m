function [U, mniter, matvecs, max_errors, history] = TDHxpKr1(K, V, Vtfun, ihfun, ui, x, tgrid, Nts, Nt_ts, Nkr, tol, Niter, Niter1st, display_mode, varargin)
...
%         end
        while (reldif>tol && ((tsi>1 && niter<Niter) || (tsi == 1 && niter<Niter1st)))
            ...
            if ~isfinite(Lambda(:, Nt_ts + 1))
                ...
                if nargout>3
                    ...
                    max_errors.FU = maxFUerror;
                    max_errors.F = maxFerror;
                    max_errors.conv = max_conv_error;
                end
                return
            end
            [Vkr, Hkr] = createKrop(@(u) Hpsi(K, Vts, u), Lambda(:, Nt_ts + 1), Nkr);
            eigval = eig(Hkr(1:Nkr, 1:Nkr));
            avgp = sum(eigval)/Nkr;
            ...
            [Unew(:, 2:(Nt_ts + 1)), Ferror, FUerror] = UfromLamb(timeMts);
        ...
        if tgrid_lowi<=tgrid_upi
            ...
            U(:, tgrid_lowi:tgrid_upi) = UfromLamb(timeMout);
        ...
        % previous time step:
        Uguess(:, 1) = Unew(:, Nt_ts);
        Uguess(:, 2:(Nt_ts + 1)) = UfromLamb(timeMnext);
        if ~isfinite(Unew(:, Nt_ts))
            % It means the solution diverges.
            % In such a case, change Nts, Nt_ts and/or Ncheb.
            fprintf('\nError: The solution diverges (in time step No. %d).\n', tsi);
            mniter = allniter/(tsi - 1);
            if nargout>3
                max_errors.texp = max_texp_error;
                max_errors.FU = maxFUerror;
    ...
    function Rv = getRv
        ...
        Rv(1, 1) = norm(Lambda(:, Nt_ts + 1));
        for spi = 1:Nkr
            Rv(1:(spi + 1), spi + 1) = (Hkr(1:(spi + 1), 1:spi)*Rv(1:spi, spi) - samplingp(spi)*Rv(1:(spi + 1), spi))/capacity;
        end
    end
    
    function [U, Ferror, FUerror] = UfromLamb(timeM)
        tp = timeM(2, :);
        Fdvd_ts = divdif(samplingp.'/capacity, Ffun(samplingp, tp, Nt_ts, tol).').';
%         Ntp = length(tp);
%         fHlamb_kr = zeros(Nkr, Ntp);
%         r = zeros(Nkr + 1, 1);
%         r(1) = norm(Lambda(:, Nt_ts + 1));
%         for spi = 1:Nkr
%             fHlamb_kr = fHlamb_kr + r(1:Nkr)*Fdvd_ts(spi, :);
%             r(1:(spi + 1)) = (Hkr(1:(spi + 1), 1:spi)*r(1:spi) - samplingp(spi)*r(1:(spi + 1)))/capacity;
%         end
        fHlamb_kr = RvKr(1:Nkr, :)*Fdvd_ts;
        U = Lambda(:, 1:(end-1))*timeM + Vkr(:, 1:Nkr)*fHlamb_kr;        
        if nargout>1
%            Ferror = abs(Fdvd_ts(Nkr + 1, Nt_ts - 1))*norm(r)/norm(fHlamb_kr(:, Nt_ts - 1));
            Ferror = abs(Fdvd_ts(Nkr + 1, Nt_ts - 1))*norm(RvKr(:, Nkr + 1))/norm(fHlamb_kr(:, Nt_ts - 1));
            FUerror = abs(Fdvd_ts(Nkr + 1, Nt_ts - 1))*norm(RvKr(:, Nkr + 1))/norm(U(:, Nt_ts - 1));
        end
    end
