function [U mniter matvecs] = TDHgeneral(H0operation, Vtoperation, ihfun, Edomain, ui, tdomain, Nts, Nt_ts, Ncheb, tol, varargin)
...
% Input:
% H0operation: the operation of the unperturbed, linear Hamiltonian. A function handle 
% of the form: @(v, more arguments). Returns the value of H0*v.
% Vtoperation: the operation of the time dependent and/or nonlinear Hamiltonian. A function handle 
% of the form: @(v, u, t, more arguments). Returns the value of Vt(u, t)*v.
...
% instead.
% Note: "more arguments" must be the same for all functions. The arguments
...
% The program is flexible, but usually, for a specific use,
% there are more efficient ways to write the algorithm.
    tinit = tdomain(1);
    tf = tdomain(2);
    ...
    for tsi = 1:Nts
        ...
        while (reldif>tol && niter<Niter)
            % Vthalf is an anonymous function, defining the operation of Vt(t = t(tmidi)) on a vector v: 
            Vthalf = @(v) Vtoperation(v, Ulast(:, tmidi), t(tmidi), varargin{:});
            % Hts_operation is an anonymous function, defining the operation of Ht(t = t(tmidi)) on a vector v:
            Hts_operation = @(v) H0operation(v, varargin{:}) + Vthalf(v);
            % Calculation of the inhomogeneous fi vectors:
            for ti = 1:Nt_ts
                Fi(:, ti) = -1i*(Vtoperation(Ulast(:, ti), Ulast(:, ti), t(ti), varargin{:}) - Vthalf(Ulast(:, ti)));
            end
            ...
            for polyi = 2:(Nt_ts+1)
                Lambda(:, polyi) = (-1i*Hts_operation(Lambda(:, polyi-1)) + Ctaylor(:, polyi-1))/(polyi - 1);
            end
            % Vcheb is a matrix. Contains the vectors:
            % Tn(Hts)*Lambda(: ,Nt_ts + 1),  n = 0, 1, ..., Ncheb-1
            % when the Tn(x) are the Chebyshev polynomials.
            ...
            Vcheb = vchebMgen(Hts_operation, Lambda(:, Nt_ts + 1), minE, maxE, Ncheb);
    ...
    mniter = allniter/Nts;
    matvecs = allniter*(3*Nt_ts + Ncheb-1);
end
