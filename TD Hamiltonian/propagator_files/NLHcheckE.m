function [U matvecs] = NLHcheckE(V, Vt, Edomain, ui, xdomain, T, Nts, Nt_ts, Ncheb, Niter1st, Niterextrap)
...
% Niter1st is the number of iterations in the 1'st time step.
% Niterextrap is the number of iterations in the other time steps, when the 1'st guess is given by an extrapolation.
% The program is based the "Chebychev propagator with iterative time ordering"
% procedure. The matrices of functions are calculated using a
% Chebychev expansion.
    Tts = T/Nts;
    tmidi = round(Nt_ts/2);
    Nx = length(ui);
...
%        matvecs = matvecs + niter*(Nt_ts + Ncheb-1);
    ...
    allniter = Niter1st + (Nts - 1)*Niterextrap;
