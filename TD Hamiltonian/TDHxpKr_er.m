function [U, mniter, matvecs, max_ers, history] = TDHxpKr_er(K, V, Vtfun, ihfun, ui, x, tgrid, Nts, Nt_ts, Nkr, Niter1st, Niter, varargin)
...
%   in each time-step.
    ...
    for tsi = 1:Nts
        ...
        while (reldif>tol && niter<Niter)
            ...
            Cnewton = devdif([t_ts, test_tpoint]*4/Tts, Fi);
            ...
            texpansion_error = norm(Fi_testpoint - Fi(:, Nt_ts + 1))/norm(Fi(:, Nt_ts + 1));
    ...
    function [U, Ferror, FUerror] = UfromLamb(timeM)
        tp = timeM(2, :);
        Fdvd_ts = devdif(samplingp.'/capacity, Ffun(samplingp, tp, Nt_ts, tol).').';
%         Ntp = length(tp);
