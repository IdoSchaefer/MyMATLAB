function U = NLHchebc(H0, minE0, maxE0, Vt, ui, x, T, Nts, Nt_ts, Ncheb, tol)
% The program solves Schrodinger equation for a nonlinear time dependent Hamiltonian.
% H0 is the linear time-independent Hamiltonian matrix.
% Vt is the nonlinear time-dependent disturbance. It is a function handle of the
% form: @(u, x, t).
% x is the grid of ui.
...
%    Ulast = zeros(dim, Nt_ts+1);    
    ...
    for tsi = 1:Nts
        ui_ts = U(:, tsi);
% Computation of the first guess: the solution within the time step, for H = H0:
        v1 = ui_ts;
        v2 = H0cheb*v1;
...
%        Ulast = U0; 
        ...
        while (reldif>tol && niter<Niter)
            ...
            for ti = 1:Nt_ts
                Fi(:, ti) = -1i*Vt(Ulast(:, ti), x, (tsi-1)*Tts + t_ts(ti)).*Ulast(:, ti);
            end
            ...
            for polyi = 2:(Nt_ts+1)
                Lambda(:, polyi) = -1i*H0*Lambda(:, polyi-1) + Ctaylor(:, polyi-1);
            end
