function U = NLHdiag1(H0, Vt, ui, x, T, Nt, Nt_ts, tol)
% Vt is a function of the form: @(u, x, t).
...
%    U(:, 1) = P\ui;
    ...
    for tsi = 1:Nts        
        ...
        H0nl = H0 + diag(Vt(ui_ts, x, (tsi-1)*Tts));
        [P, D] = eig(H0nl);
        eigval = diag(D);
        expeigval = exp(-1i*eigval*(dt:dt:Tts));
        ui_tsD = P\ui_ts;
        for ti = 1:Nt_ts
            U0(:, ti) = expeigval(:, ti).*ui_tsD;
        end
        Ulast = U0;
        niter = 0;
        Vt0 = Vt(ui_ts, x, (tsi - 1)*Tts);
        reldif = tol + 1;
        while (reldif>tol && niter<Niter)
            Unew = U0;
            for taui = 1:Nt_ts
                Ulastx = P*Ulast;
                vtau = -1i*(P\((Vt(Ulastx(:, taui), x, (tsi - 1)*Tts + taui*dt) - Vt0).*Ulastx(:, taui)))*dt;
        ...
        %niter
        U(:, (tsi-1)*Nt_ts + 1 + (1:Nt_ts)) = P*Unew;
    end
%     U(:, 1) = ui;
%     U(:, 2:Nt+1) = P*U(:, 2:Nt+1);
end