function [U, mniter, matvecs, max_errors, history] = SemiGlobalDirty(Gop, Gdiff_op, Gdiff_matvecs,...
...
%   Setting defaults:
    ...
    % The test point in the theta domain:
    Nt_ts_is_even = (mod(Nt_ts, 2) == 0);
    if Nt_ts_is_even
        texp_factor_even = 8*abs(Tts/((Nt_ts^2 - 1)*(Nt_ts - 3)));
        % s_ext evaluation time points for error estimation:
        texp_er_tpoints = (t_ts(tmidi) + t_ts(tmidi + 1))/2;
        texp_er_i = Nt_ts + 1;
    else
        texp_factor_odd = 4*abs(Tts*(Nt_ts - 1)/(Nt_ts*(Nt_ts^2 - 4)*(Nt_ts - 4)));
        % Note that this differs by a 1/Tts factor from texpEfactor2 in
        % TDHxp_tests. Be careful.
        % s_ext evaluation time points for error estimation:
    ...
    texp_factor = 2*abs(Tts*sin((Nt_ts - 1)*tp_theta)*(cos(2*tp_theta) - 1)/sin(tp_theta));
    % Actually, this is very close to 4*Tts, so I'm not sure that this
    % computation is worth the effort - the order is the same.
    ...
    if ~Arnoldi
        ...
        % A more elegant alternative, by the next order term in a Newton
        % expansion:
%         fm_cheb_samplingp = -cos([(2*(0:(Nfm - 1)) + 1)*pi/(2*Nfm), round(Nfm/2)*pi/Nfm]);
%         fm_sp = (fm_cheb_samplingp*(max_ev - min_ev) + min_ev + max_ev)/2;
%         dvd_fz4 = divdif(2*fm_cheb_samplingp, f_fun(fm_sp.', Tts).');
%         %Actually, if the divided differences are anyway computed, it's better
%         to use an interpolation at the Chebyshev points, rather than
%         computing the Chebyshev polynomial coefficients.
%         f_scalar_error = abs(dvd_fz4(Nfm + 1))*2;
%         if nargout>3
...
%         end
    ...
    if nargout>4
        ...
        history.texp_error.cheap = zeros(1, Nts);
        %history.texp_error.cheap1 = zeros(1, Nts);
        history.texp_error.exact = zeros(1, Nts);
        history.f_error = zeros(1, Nts);
        history.fUerror = zeros(1, Nts);
        history.reldif = zeros(1, Nts);
        history.conv_error.cheap = zeros(1, Nts);
        history.conv_error.exact = zeros(1, Nts);
    ...
    %Gdiff_u_new = zeros(Nu, Nt_ts);
    % The indices for the computation of s_ext (excluding tmidi):
    s_ext_i = [1:(tmidi - 1), (tmidi + 1):total_Nt_ts];
    ...
    max_fUerror = 0;
    max_conv_error = 0;
    % Chebyshev integration weights for caculation of
    % history.conv_error.exact:
    ...
    for tsi = 1:Nts
        ...
        Gdiff_u_new = Gdiff_op(Ulast(:, s_ext_i(1:(Nt_ts - 1))), t(s_ext_i(1:(Nt_ts - 1))), Ulast(:, tmidi), t(tmidi), varargin{:});
        ...
        while (reldif>tol && ((tsi>1 && niter<Niter) || (tsi == 1 && niter<Niter1st)))
            ...
            s_ext(:, s_ext_i(1:(Nt_ts - 1))) = Gdiff_u_new;
            s_ext(:, s_ext_i(Nt_ts:(total_Nt_ts - 1))) =...
                Gdiff_op(Ulast(:, s_ext_i(Nt_ts:(total_Nt_ts - 1))), t(s_ext_i(Nt_ts:(total_Nt_ts - 1))),...
                Ulast(:, tmidi), t(tmidi), varargin{:});
            ...
            Cnewton = divdif(t_2ts(1:total_Nt_ts)*4/Tts, s_ext); %%% changed %%%
...
%             for ti = (Nt_ts-1):-1:1
%                 s_ext_testpoint = Cnewton(:, ti) + (4/Tts)*(texp_er_tpoints(1) - t_ts(ti))*s_ext_testpoint;
%             end
            % The error estimation of the time-expansion:
            normUnew = norm(Unew(:, Nt_ts));
            %texpansion_error = norm(s_ext_testpoint - s_ext(:, Nt_ts + 1))*abs(Tts)/normUnew;
%            % Alternative computation:
            normCnewton_t_er = norm(Cnewton(:, Nt_ts + 1));
            texpansion_error = texp_factor*normCnewton_t_er/normUnew;
            % Convergence check of u:
            reldif = norm(Unew(:, Nt_ts) - Ulast(:, Nt_ts))/norm(Ulast(:, Nt_ts));
            % Calculation of the new inhomogeneous s_ext vectors (without
            % the source term s(t)):
            Gdiff_u_new = Gdiff_op(Unew(:, s_ext_i(1:(Nt_ts - 1))), t(s_ext_i(1:(Nt_ts - 1))), Unew(:, tmidi), t(tmidi), varargin{:});
            if there_is_ih
                conv_error_cheap = norm(Gdiff_u_new(:, Nt_ts - 1) - Gdiff_u(:, Nt_ts - 1))*Tts/norm(Unew(:, Nt_ts));
                conv_error_exact = norm((Gdiff_u_new - Gdiff_u)*wcheb)/norm(Unew(:, Nt_ts));
            else
                conv_error_cheap = norm(Gdiff_u_new(:, Nt_ts - 1) - s_ext(:, Nt_ts))*Tts/norm(Unew(:, Nt_ts));
                conv_error_exact = norm((Gdiff_u_new - s_ext(:, s_ext_i(1:(Nt_ts - 1))))*wcheb)/norm(Unew(:, Nt_ts));
            end
            Ulast = Unew;
            niter = niter + 1;
            % Divergence investigation:
%             fprintf('Time-step: %d, Iteration No. %d\n', tsi, niter)
%             fprintf('Solution norm:\n')
%             vecnorm(Unew)
%             fprintf('Ctaylor/lambda^m:\n')
%             vecnorm(Ctaylor).*cumprod(1:Nt_ts)./(max(abs(ev_domain)).^(1:Nt_ts))
        end
        if display_mode
            if reldif>tol
        ...
        if nargout>4
            history.texp_error.cheap(tsi) = texpansion_error;
            %history.texp_error.cheap1(tsi) = texpansion_error;
            if Nt_ts_is_even
                history.texp_error.exact(tsi) = texp_factor_even*normCnewton_t_er/normUnew;
            else
                history.texp_error.exact(tsi) =...
                    texp_factor_odd*norm(4*Cnewton(:, Nt_ts + 2) - ...
            ...
            history.conv_error.cheap(tsi) = conv_error_cheap;
            history.conv_error.exact(tsi) = conv_error_exact;
        ...
        if tsi ~= 1
            ...
            % Maybe this should be updated in accordance with the error
            % estimation cost?
        end
...
%         vecnorm(Uguess)
    ...
    if nargout>4
        history.total_errors.texp_cheap = sum(history.texp_error.cheap);
        history.total_errors.texp_exact = sum(history.texp_error.exact);
        history.total_errors.fU = sum(history.fUerror);
        history.total_errors.conv_cheap = sum(history.conv_error.cheap);
        history.total_errors.conv_exact = sum(history.conv_error.exact);
        history.total_errors.total = history.total_errors.texp_exact + history.total_errors.fU + history.total_errors.conv_exact;
    end
    ...
    function [U, f_error, fUerror] = Ufrom_vCheb(v_vecs, timeM, Vcheb, Ccheb_f)
        fGv = Vcheb*Ccheb_f;
        U = v_vecs(:, 1:Nt_ts)*timeM + fGv;
        if nargout>1
            f_abs_error = f_scalar_error*norm(v_vecs(:, Nt_ts + 1));
            f_error = f_abs_error/norm(fGv(:, Nt_ts - 1));
