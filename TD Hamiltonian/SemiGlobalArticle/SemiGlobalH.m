 function [U, mniter, matvecs, max_errors, history] = SemiGlobalH(Hop, Hdiff_op, Hdiff_matvecs, ihfun, ev_domain, ui, tgrid, Nts, Nt_ts, Nfm, tol, Niter, Niter1st, display_mode, varargin)
...
%
% Input:
% Hop: A function handle of the form @(u, t, v, more arguments). Returns 
% H(u, t)v. The "more arguments" will be written in the place of "varargin" in this program.
% Hdiff_op: A function handle of the form @(u1, t1, u2, t2, more arguments). Returns
% (H(u1, t1) - H(u2, t2))u1. u1 and t1 represent several time-points in 
...
%   Setting defaults:
    ...
    for tsi = 1:Nts
        ...
        while (reldif>tol && ((tsi>1 && niter<Niter) || (tsi == 1 && niter<Niter1st)))
            ...
            % calculated.
            s_ext(:, s_ext_i) = -1i*Hdiff_op(Ulast(:, s_ext_i), t(s_ext_i), Ulast(:, tmidi), t(tmidi), varargin{:});
            ...
            for polyi = 2:(Nt_ts+1)
                v_vecs(:, polyi) = (-1i*Hop(Ulast(:, tmidi), t(tmidi), v_vecs(:, polyi-1), varargin{:}) + Ctaylor(:, polyi-1))/(polyi - 1);
            end
            ...
            if Arnoldi
                ...
                [Upsilon, Hessenberg] = createKrop(@(v) Hop(Ulast(:, tmidi), t(tmidi), v, varargin{:}), v_vecs(:, Nt_ts + 1), Nfm);
                ...
                Vcheb = vchebMop(@(v) Hop(Ulast(:, tmidi), t(tmidi), v, varargin{:}), v_vecs(:, Nt_ts + 1), min_ev, max_ev, Nfm);
