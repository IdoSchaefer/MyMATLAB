function [U, mniter, matvecs, max_errors, history] = SemiGlobalArnoldi_xp1(K, V, Vtfun, ihfun, ui, x, tgrid, Nts, Nt_ts, Nkr, tol, Niter, Niter1st, display_mode, varargin)
...
%   Setting defaults:
    ...
    t_tsLeja = [0, Tts, zeros(1, Nt_ts - 2)];
    [t_tsLeja(3:Nt_ts), originali] = Lejap1([0, Tts], t_ts(2:(Nt_ts - 1)), Nt_ts - 2, Tts/4);
    if nargout>4
        ordered2Lejai = [1, Nt_ts, originali + 1];
        [~, Leja2orderedi] = sort(ordered2Lejai);
    end
    tmidiLeja = 3;
    tmaxiLeja = 2;
    ...
    t_2ts = [t_tsLeja, test_tpoint, Tts + t_tsLeja(2:Nt_ts), Tts + test_tpoint];
    %t4samplingp = t_2ts(1:(Nt_ts + 1))*4/Tts;
    t4samplingp = t_tsLeja*4/Tts;
    sext2newtonM = f2dvdM(t4samplingp);
    if nargout>4
    ...
    Cr2t = r2Taylor4(t_tsLeja, Tts);
    % Cr2t = r2Taylor4(t4samplingp(1:Nt_ts)*Tts/4, Tts);
    % The extended "inhomogeneos" vectors:
    s_ext = zeros(Nx, Nt_ts + 1);
    % The indices for the computation od s_ext (excluding tmidiLeja):
    s_ext_i = [1:(tmidiLeja - 1), (tmidiLeja + 1):(Nt_ts + 1)];
    ...
    for tsi = 1:Nts
        ...
        t = tinit + Tts*(tsi - 1) + [t_tsLeja, test_tpoint];
        ...
        while (reldif>tol && ((tsi>1 && niter<Niter) || (tsi == 1 && niter<Niter1st)))
            % Vthalf is the x vector representing Vt(t = t(tmidiLeja)): 
            Vthalf = Vtfun(Ulast(:, tmidiLeja), x, t(tmidiLeja), varargin{:});
            % Vts is the x vector representing the total potential energy 
            % operator at t = t(tmidiLeja):
            Vts = V + Vthalf;
            % Calculation of the inhomogeneous s_ext vectors. Note that
            % s_ext(:, tmidiLeja) is equivalent to s(:, tmidiLeja), and therefore not
            % calculated.
            for ti = s_ext_i
            ...
            % interpolation at the points t_tsLeja.
            ...
            %Cnewton = s_ext*sext2newtonM;
            %Cnewton = divdif(t4samplingp, s_ext(:, 1:Nt_ts));
            Cnewton = s_ext(:, 1:Nt_ts)*sext2newtonM;
            if tsi == 2454
                keyboard
            end
...
%            [Upsilon, Hessenberg] = createKrop(@(u) -1i*Hpsi(K, Vts, u), v_vecs(:, Nt_ts + 1), Nkr);
            ...
            %samplingp = Lejap1([], [eigval; avgp], Nkr + 1, capacity);
            ...
            for ti = (Nt_ts-1):-1:1
                s_ext_testpoint = Cnewton(:, ti) + (4/Tts)*(test_tpoint - t_tsLeja(ti))*s_ext_testpoint;
                %s_ext_testpoint = Cnewton(:, ti) + ((4/Tts)*test_tpoint - t4samplingp(ti))*s_ext_testpoint;
            end
            % The error estimation of the time-expansion:
            texpansion_error = norm(s_ext_testpoint - s_ext(:, Nt_ts + 1))*Tts/norm(Unew(:, Nt_ts + 1));
            ...
            reldif = norm(Unew(:, tmaxiLeja) - Ulast(:, tmaxiLeja))/norm(Ulast(:, tmaxiLeja));
            Ulast = Unew;
            niter = niter + 1;
        end
        ...
        while tgrid_upi<Nt && (t(tmaxiLeja) - tgrid(tgrid_upi + 1))*direction>abs(t(tmaxiLeja))*eps*10
        ...
        % propagation grid:
        if abs(t(tmaxiLeja) - tgrid(tgrid_upi + 1))<=abs(t(tmaxiLeja))*eps*10
            tgrid_upi = tgrid_upi + 1;
            U(:, tgrid_upi) = Unew(:, tmaxiLeja);
            tgrid_lowi = tgrid_upi + 1;
        end
        if nargout>4
            %history.U(:, ((tsi - 1)*(Nt_ts - 1) + 2):(tsi*(Nt_ts - 1) + 1)) = Unew(:, 2:Nt_ts);
            history.U(:, ((tsi - 1)*(Nt_ts - 1) + 2):(tsi*(Nt_ts - 1) + 1)) = Unew(:, Leja2orderedi(2:Nt_ts));
        end
        % The new guess is an extrapolation of the solution within the
        % previous time step:
        Uguess(:, 1) = Unew(:, tmaxiLeja);
    ...
    function [U, f_error, fUerror] = Ufrom_v(timeM)
        ...
        %fdvd_ts = divdif_mat(samplingp.'/capacity, f_fun(samplingp, tp, Nt_ts, tol).').';
