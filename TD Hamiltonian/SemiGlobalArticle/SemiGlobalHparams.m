function [U, mniter, matvecs, max_errors, history] = SemiGlobalHparams(Hop, Hdiff_op, Hdiff_matvecs, Hparams, ihfun, ev_domain, ui, tgrid, Nts,...
    Nt_ts, Nfm, tol, Niter, Niter1st, test_tpoint, display_mode, varargin)
...
% by Hillel Tal-Ezer. More convenient for quantum applications than
% the program SemiGlobal.
% There is an option to include an inhomogeneos term.
% This version is suitable in problems in which the time-dependence of the
% Hamiltonian consists solely on a small set of time-dependent parameters,
% and in addition, it is advantageous to evaluate all these parameters in
% advance, before the propagation. It is useful also in inhomogeneous problems
% in which it is advantageous to evaluate the time-dependent inhomogenuity
% in advance.
%
% Input:
...
% Hdiff_matvecs: The number of matrix-vector multiplications required for
% the computation of the operation of Hdiff_op for each time-point (usually less than 2).
...
% problem.)
% Hparams: Can be a function handle or a matrix; represents the
% time-dependent Hamiltonian parameters. If Hparams is a function handle,
% it has the form: @(t, more arguments), where t is a row vector of time-points.
% "more arguments" must be the same as for Hop. The output is a matrix
% which contains the time-dependent parameters evaluated at all the
% time-points contained in the input row vector. The column index indexes 
% the time-points, while the row index indexes the different time-dependent parameters.
% It is possible to insert additional arguments in the form: @(t, more arguments). 
% If Hparams is a matrix, it contains the time-dependent parameters in the
% same manner as above, in the time points defined by the variable propagation_grid
% in the present code.
...
% instead.
% Note: "more arguments" must be the same as for Hop.
% It is possible to insert a data matrix instead, in the same manner as for
% Hparams.
% ev_domain: The boundaries of the eigenvalue domain of H, when a Chebyshev
...
%   history.Utestp: The solution at the test-points
...
%   Setting defaults:
    ...
    if nargin<15 || isempty(test_tpoint)
        % The default test-point for error estimation:
    ...
    % For the function f_fun:
    factorialNt_ts = factorial(Nt_ts);
    % If Hparams is a function handle:
    if length(Hparams) == 1
        Hparams_pgrid = Hparams(propagation_grid, varargin{:});
    else
        % If Hparams is a data array:
        Hparams_pgrid = Hparams;
    end
    if length(ihfun) == 1
    % If ihfun is a function handle:
        s_pgrid = ihfun(propagation_grid, varargin{:});
    elseif length(ihfun)>1
    % If ihfun is a data array:
        s_pgrid = ihfun;
    end
    if ~Arnoldi
...
%         fprintf('Stability factor: %d\n', f_scalar_error*max(abs(ev_domain))^Nt_ts/factorialNt_ts);
    ...
    if nargout>4
        ...
        history.U(:, 1) = ui;
        % The U evaluated at the test-points in each time-step:
        history.Utestp = zeros(Nu, Nts);
        %history.t = [kron(tinit:Tts:(tf - Tts), ones(1, Nt_ts - 1)), tf] + [kron(ones(1, Nts), t_ts(1:(Nt_ts - 1))), 0];
        history.t = propagation_grid;
        history.t(Nt_ts:Nt_ts:(Nts*Nt_ts)) = [];
    ...
    max_conv_error = 0;
    for tsi = 1:Nts
        % The indices of the current time-step in the full propagation
        % grid:
        ts_indices = (tsi - 1)*Nt_ts + [(1:(Nt_ts - 1)), Nt_ts + 1, Nt_ts];
        % The time of the interior time points within the time-step:
        t = propagation_grid(ts_indices);
        % Creating a matrix which contains the time-dependent parameters
        % evaluated at the time-points defined by t:
        Hparams_ts = Hparams_pgrid(:, ts_indices);
        ...
        v_vecs(:, 1) = Ulast(:, 1);
        if there_is_ih
            % The inhomogeneous term at the time-points defined by t:
            s = s_pgrid(:, ts_indices);
        end
        ...
        while (reldif>tol && ((tsi>1 && niter<Niter) || (tsi == 1 && niter<Niter1st)))
            ...
            % calculated.
            s_ext(:, s_ext_i) = -1i*Hdiff_op(Ulast(:, s_ext_i), Hparams_ts(:, s_ext_i), Ulast(:, tmidi), Hparams_ts(:, tmidi), varargin{:});
            ...
            for polyi = 2:(Nt_ts+1)
                v_vecs(:, polyi) = (-1i*Hop(Ulast(:, tmidi), Hparams_ts(:, tmidi), v_vecs(:, polyi-1), varargin{:}) + Ctaylor(:, polyi-1))/(polyi - 1);
            end
            ...
            if Arnoldi
                ...
                [Upsilon, Hessenberg] = createKrop(@(v) Hop(Ulast(:, tmidi), Hparams_ts(:, tmidi), v, varargin{:}), v_vecs(:, Nt_ts + 1), Nfm);
                ...
                Vcheb = vchebMop(@(v) Hop(Ulast(:, tmidi), Hparams_ts(:, tmidi), v, varargin{:}), v_vecs(:, Nt_ts + 1), min_ev, max_ev, Nfm);
        ...
        if tsi ~= 1
            ...
            if Arnoldi
                matvecs = niter*(Nt_ts*(1 + Hdiff_matvecs) + Nfm);
            else
                matvecs = niter*(Nt_ts*(1 + Hdiff_matvecs) + Nfm - 1);
            end
        end
        ...
        if nargout>4
            ...
            history.Utestp(:, tsi) = Unew(:, Nt_ts + 1);
        end
    ...
    if nargout>2
        if Arnoldi
            matvecs = matvecs + allniter*(Nt_ts*(1 + Hdiff_matvecs) + Nfm);
        else
            matvecs = matvecs + allniter*(Nt_ts*(1 + Hdiff_matvecs) + Nfm - 1);
        end
    end
    ...
    % The function f(-1i*z, t):
        ...
        result = ones(Nz, Ntp);
        % First, we compute f(-1i*z, t)/(t^Nt_ts), which is a function of -izt.
        % A direct computation for large arguments:
        result(is_big) = exp(minus_izt(is_big));
        for polynomi = 1:Nt_ts
            result(is_big) = polynomi*(result(is_big) - 1)./minus_izt(is_big);
        end
