function [U, mniter, matvecs, max_errors, history] = SemiGlobal2020(Gop, Gdiff_op, Gdiff_matvecs, ihfun, ev_domain, ui, tgrid, Nts, Nt_ts, Nfm, tol, Niter, Niter1st, display_mode, varargin)
...
%   Setting defaults:
    ...
    if ~Arnoldi
        ...
        fztest = f_fun(ztest, Tts, Nt_ts, tol);
    ...
    if nargout>4
        ...
        if Arnoldi
            ...
            % fiunction of matrix computation is different for each
            % time-step:
...
%             for ti = s_ext_i
%                 s_ext(:, ti) = Gdiff_op(Ulast(:, ti), t(ti), Ulast(:, tmidi), t(tmidi), varargin{:});
%             end
            ...
            if Arnoldi
                ...
                [Unew(:, 2:(Nt_ts + 1)), f_error, fUerror] = Ufrom_vArnoldi(timeMts);
            else
                ...
                % within the time step:
                [Unew(:, 2:(Nt_ts + 1)), fUerror] = Ufrom_vCheb(timeMts, Ccheb_f_ts);
            end
        ...
        if tgrid_lowi<=tgrid_upi
            ...
            if Arnoldi
                U(:, tgrid_lowi:tgrid_upi) = Ufrom_vArnoldi(timeMout);
            else
                Ccheb_f_out = f_chebC(tgrid(tgrid_lowi:tgrid_upi) - t(1));
                U(:, tgrid_lowi:tgrid_upi) = Ufrom_vCheb(timeMout, Ccheb_f_out);
            end
        ...
        Uguess(:, 1) = Unew(:, Nt_ts);
        if Arnoldi
            Uguess(:, 2:(Nt_ts + 1)) = Ufrom_vArnoldi(timeMnext);
        else
            Uguess(:, 2:(Nt_ts + 1)) = Ufrom_vCheb(timeMnext, Ccheb_f_next);
        end
        if there_is_ih
            s(:, 1) = s(:, Nt_ts);
        end
    ...
    function [U, f_error, fUerror] = Ufrom_vArnoldi(timeM)
    ...
    function [U, fUerror] = Ufrom_vCheb(timeM, Ccheb_f)
        fGv = Vcheb*Ccheb_f;
