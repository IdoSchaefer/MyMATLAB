function [allNt, allmv, aller, max_ers] = errorSGarticle_pos(T, Nt_ts, Nkr, minNt, Nsamp, Niter)
...
for degi = 1:Nsamp
    ...
    allNt(degi) = Nt;
    %dt = T/Nt
    [u, ~, matvecs, all_est_er] = TDHxpKr1(Kp, Vabsp, @(u,x,t) -xabsp*0.1*sech(-(t-500)/(170)).^2.*cos(0.06*(t-500)), [], fi0p, xp, [0 T], Nt, Nt_ts, Nkr, eps, Niter, 20, false);
