function [U, mniter, matvecs, est_errors, history] = SemiGlobal2(Gop, Gdiff_op, Gdiff_matvecs,...
...
%   expensive numerically. Not computed for odd Nt_ts if options.texp_er_odd = false
...
%   The time-expansion error estimations for all time-steps (see the
...
%   Setting defaults:
    ...
    if nargout>4
        ...
        history.fm_error_pre = zeros(1, Nts);
    ...
    factorial_Nt_ts_plus_Nfm = factorial(Nt_ts + Nfm); %It's a waste. We should use cumprod in the final version.
    if ~Arnoldi
    ...
    for tsi = 1:Nts
        ...
        while (tsi>1 && niter<options.Niter && ((tol_mode && conv_error>tol_ts) || ~tol_mode)) ||...
            ...
            fUerror_pre =...
                abs(Tts)^(Nt_ts + Nfm)*factorialNt_ts/factorial_Nt_ts_plus_Nfm*capacity^Nfm*norm(v_vecs(:, Nt_ts + 1))/norm(Ulast(:, 1));
            if ~Arnoldi
                fUerror_pre = fUerror_pre*2;
            end
        ...
        est_errors.fm_pre = est_errors.fm_pre + fUerror_pre;
        ...
        if nargout>4
            ...
            history.fm_error_pre(tsi) = fUerror_pre;
