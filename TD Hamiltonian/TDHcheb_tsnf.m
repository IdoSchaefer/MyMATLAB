function [U mniter matvecs] = TDHcheb_tsnf(V, Vt, Edomain, ui, xdomain, T, Nts, Nt_ts, Ncheb, tol)
...
% procedure. The functions of matrices are caculated using a
...
%        Ulast = U0; 
        ...
        while (reldif>tol && niter<Niter)
            ...
            for ti = 1:Nt_ts
                Fi(:, ti) = -1i*(Vt(x, (tsi-1)*Tts + t_ts(ti)) - Vhalfv).*Ulast(:, ti);
            end
