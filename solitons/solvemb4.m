clf
syms F V r real;
funchoice = input('Choose: a potential/force function(V/F), or an alternating potential(a):', 's');
if funchoice == 'V'
...
if mdqsn ~= 'y' && mdqsn ~= 'Y'
    ...
    initmd = input('Choose the masses initialization: as your choice, random according to the abundance, \n as a periodic structure, as a function (c, r, p, f)?', 's');
    switch initmd
        case 'r'
            sm = input('Enter the standard mass: ');
            m = ones(1, N)*sm;
            md = input('Enter the first kind of mass defect:');
            abn = input('Enter the abundance (precent, 0 to continue): ', 's');
            mi = 1;
            while abn >= 0 && abn <= 100               
                abmd.m(mi) = md;
                abmd.ab(mi) = abn;
                mi = mi + 1;
                md = input('Enter another kind of mass defect:');
                abn = input('Enter the abundance (precent, 0 to continue): ', 's');
            end
        case 'p'
            'Enter the masses pattern in the correct order: \n'
            mpi = 1;
            mpn = input('Enter the first mass: ');
            while mpn(mpi)>=0 && mpi<=N
                mpi = mpi + 1;
                mpn(mpi) = input('Enter the next mass (0 for the end of the pattern): ');
            end
            mi = 1;
