clf
syms F V Vm r xi real;
getV = input('Enter the potential function of the chain in the terms of r: ', 's');
V = eval(getV);
F = -diff(V);
getVm = input('Enter the potential function of the molecule''s bond in the terms of xi: ', 's');
Vm = eval(getVm);
Fm = -diff(Vm);
d = input('Enter the damping coefficient: ');
...
if boundary == 'f'
    ...
    sol = ode15s(@fixedcexpd, [0 tfinal], [initx initv], [], F, Fm, r, xi, r0, m, N, d);
elseif boundary == 'c'
    if initchoice == 's'
    ...
    sol = ode45(@cycliccexp, [0 tfinal], [initx initv], [], F, Fm, r, xi, r0, m, N);
else
    if initchoice == 's'
        mode = input('Which mode?');
        A = input('Enter the amplitude:');
    ...
    sol = ode45(@opencexp, [0 tfinal], [initx initv], [], F, Fm, r, xi, r0, m, N);
end
dt = 0.01;
t = 0:dt:tfinal;
x = deval(sol, t);
